...
class Config(SerializableDataclass):
	...
	exclude_if_true: tuple[str] = serializable_field(
		default=("_done", "cancelled", "backlog")
	)  # checks for these keys in `event.data`
	glob_regex_exclude: tuple[str] = serializable_field(
		default=tuple()
	)  # exclude a file if it matches
	...
	def load(cls, data: dict) -> "Config":
		return cls(
			dendron_event_tags=tuple(
				set(data.get("dendron_event_tags", cls.dendron_event_tags))
			),
			exclude_if_true=tuple(
				set(data.get("exclude_if_true", cls.exclude_if_true))
			),
			glob_regex_exclude=tuple(
				set(data.get("glob_regex_exclude", cls.glob_regex_exclude))
			),
			markdown_output_format=data.get(
				"markdown_output_format", cls.markdown_output_format
			),
			ical_title_fmt=data.get("ical_title_fmt", cls.ical_title_fmt),
			ical_description_fmt=data.get(
				"ical_description_fmt", cls.ical_description_fmt
			),
			description_delimiter=data.get(
				"description_delimiter", cls.description_delimiter
			),
		)
...
def custom_dateparse(s: str) -> date | datetime:
	...
	if s.lower() in {"today", "tod"}:
		...
		if dt is None:
			warn(
				f"got datetime as None for '{s}' in custom_dateparse, setting to datetime.now()"
			)
			dt = datetime.now()
...
class Event(SerializableDataclass):
	...
	time_start: datetime | date | None = serializable_field(
		...
		loading_fn=lambda d: custom_dateparse(d["time_start"])
		if d.get("time_start", None) is not None
		else None,
	)
	duration: timedelta | None = serializable_field(
		serialization_fn=lambda d: d.total_seconds() if d is not None else None,
		loading_fn=lambda d: timedelta(seconds=d["duration"])
		if d.get("duration", None) is not None
		else None,
	)
	...
	origin_file_dendron = property(
		lambda self: _markdown_output_process_path_dendron(self.data["_file"])
	)
	origin_line = property(lambda self: self.data["_line"])
	origin_vscode_link = property(
		lambda self: f"vscode://file/{Path(os.getcwd()) / self.origin_file}:{self.origin_line}"
	)
	tag_dendron = property(
		lambda self: f"tags.{self.tag.lstrip('#')}" if self.tag is not None else None
	)
	tag_stripped = property(
		lambda self: ".".join(self.tag.split(".")[1:]) if self.tag is not None else None
	)

	time_start_readable = property(
		lambda self: date_to_datetime(self.time_start).isoformat()
		if self.time_start is not None
		else None
	)
	duration_readable = property(
		lambda self: str(self.duration) if self.duration is not None else None
	)
	time_end_readable = property(
		lambda self: date_to_datetime(self.time_end).isoformat()
	...
	@classmethod
	def from_de_dict(
		cls, data: dict, default_duration: timedelta = parse_as_delta("30 min")
	) -> "Event":
		...
		if "due" in data:
			...
			if isinstance(time_start, datetime):
				...
				raise TypeError(
					f"Invalid type for time_start: {type(time_start) = } {time_start = }"
				)
		...
		if ("title" in data) and ("description" in data):
			...
			raise ValueError(
				f"Either both title and description must be specified, or neither. {data = }"
			)
	...
	def to_ical_dict(self) -> dict:
		...
		return {
			...
			"DTSTART": ical_datetime_format(self.time_start)
			if self.time_start is not None
			else today_date_str,
			"DTEND": ical_datetime_format(self.time_end)
			if self.time_start is not None
			else today_date_str,
				...
				if event is not None:
					...
					if line.rstrip().endswith(CFG.description_delimiter):
						event["title"] = (
							event["_content"]
							.rstrip()
							.rstrip(CFG.description_delimiter)
							.rstrip()
						)
...
def glob_get_dendron_events(
	...
	regexes_exclude: list[re.Pattern] = [
		re.compile(pattern) for pattern in CFG.glob_regex_exclude
	]
...
def ical_serialize_dict(d: dict, bounder: str | None = None) -> str:
	...
	for k, v in d.items():
		if isinstance(v, (str, int, float)):
			...
			joined_dict_items: str = ";".join(
				[f"{k2}={ical_process_value(v2)}" for k2, v2 in v.items()]
			)
...
def create_md_content(events: list[Event]) -> str:
	...
	tag_grouped: dict[str, list[Event]] = {
		tag: [e for e in events if e.tag == tag] for tag in tags_found
	}

	# sort the events by time
	tag_grouped_sorted: dict[str, list[Event]] = dict()
	for tag, e_list in tag_grouped.items():
		tag_grouped_sorted[tag] = sorted(
			e_list,
			key=lambda e: date_to_datetime(e.time_start)
			if e.time_start is not None
			else DISTANT_FUTURE_TIME,
		)
...
def filter_events(events: list[Event], cfg: Config = CFG) -> list[Event]:
	...
	return [
		e
		for e in events
		if not any(bool_str_parse(e.data.get(x, False)) for x in cfg.exclude_if_true)
	]
...
if __name__ == "__main__":
	if any(arg.lower().lstrip("-") in {"help", "h"} for arg in sys.argv):
		print(__doc__)
		sys.exit(0)
	...
	fire.Fire(
		{
			"json": lambda path, cfg_path=None: events_to_json(
				general_loader(path, cfg_path)
			),
			"jsonl": lambda path, cfg_path=None: events_to_json(
				general_loader(path, cfg_path), jsonl=True
			),
			"ical": lambda path, cfg_path=None: create_ical_content(
				general_loader(path, cfg_path)
			),
			"md": lambda path, cfg_path=None: create_md_content(
				general_loader(path, cfg_path)
			),
			"markdown": lambda path, cfg_path=None: create_md_content(
				general_loader(path, cfg_path)
			),
